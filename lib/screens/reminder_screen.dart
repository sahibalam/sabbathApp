...
// Enhanced audio playback for full duration
Future<void> _playFullAlarm() async {
  ...
  try {
    ...
    // Play WITHOUT loop mode to ensure full duration playback
    ...
    // Show dismissible dialog for user control
    _showAlarmDialog();
    ...
    debugPrint('✅ Full alarm started - will play complete audio file');
...
void _showAlarmDialog() {
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: const Text('🕯️ Sabbath Reminder'),
        content: const Text('Your Sabbath reminder is playing.\nTap "Stop" to dismiss.'),
        ...
        actions: [
          TextButton(
            ...
            child: const Text('Stop', style: TextStyle(color: Color(0xFFF4732F))),
          ),
        ],
      );
    },
   ...
   if (testSebastianMode) {
      ...
      await notificationPlugin.initialize(
        ...
        onDidReceiveNotificationResponse: (details) async {
          ...
          if (details.payload == 'play_full_alarm') {
            await _playFullAlarm();
          }
        },
      );
...
Future<void> _scheduleAllReminders() async {
  ...
  for (final entry in reminderTimes.entries.where((e) => e.value)) {
    ...
    if (startReminderTime.isAfter(now)) {
      await _scheduleWithVerification(
        ...
        body: 'Sabbath begins in $minutes minutes. Tap to hear full reminder.',
    ...
    if (endReminderTime.isAfter(now)) {
      await _scheduleWithVerification(
        ...
        body: 'Sabbath ends in $minutes minutes. Tap to hear full reminder.',
  ...
  if (nextStart.isAfter(now)) {
    await _scheduleWithVerification(
      ...
      body: 'Shabbat Shalom! Tap to hear the full reminder.',
  ...
  if (nextEnd.isAfter(now)) {
    await _scheduleWithVerification(
      ...
      body: 'Have a blessed week! Tap to hear the full reminder.',
    ...
    await notificationPlugin.zonedSchedule(
      ...
      NotificationDetails(
        android: AndroidNotificationDetails(
          ...
          playSound: false, // 🔇 DISABLED - Using AudioPlayer instead
        ...
        iOS: const DarwinNotificationDetails(
          presentAlert: true,
          presentBadge: true,
          presentSound: false, // 🔇 DISABLED - Using AudioPlayer instead
          interruptionLevel: InterruptionLevel.timeSensitive,
        ),
      ),
      androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
      payload: 'play_full_alarm', // 🎵 Triggers full audio playback
