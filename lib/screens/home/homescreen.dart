...
class _HomeScreenState extends State<HomeScreen>
  ...
  Widget _sabbathSection() {
    ...
    // Get the Sabbath dates based on current state
    DateTime mainSabbathStart = _sabbathHasEnded
        ? _getNextSabbathStart(now, currentOffset)
        : _getCurrentSabbathStart(now, currentOffset);

    DateTime mainSabbathEnd = _sabbathHasEnded
        ? _getNextSabbathEnd(now, currentOffset)
        : _getCurrentSabbathEnd(now, currentOffset);

    // Always get the next Sabbath after the main one
    DateTime nextSabbathStart = _getNextSabbathStart(
      mainSabbathStart,
      currentOffset,
    );
    DateTime nextSabbathEnd = _getNextSabbathEnd(
      mainSabbathStart,
      currentOffset,
    );

    // Format the dates and times
    final startDateText = _formatSabbathDate(mainSabbathStart);
    final endDateText = _formatSabbathDate(mainSabbathEnd);
    final nextStartDateText = _formatSabbathDate(nextSabbathStart);
    final nextEndDateText = _formatSabbathDate(nextSabbathEnd);

    final startSunsetText = _getSunsetTime(mainSabbathStart, currentOffset);
    final endSunsetText = _getSunsetTime(mainSabbathEnd, currentOffset);
