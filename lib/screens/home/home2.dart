...
class _HomeScreenState extends State<HomeScreen>
  ...
  DateTime _getCurrentSabbathStart(DateTime now, [Duration? offset]) {
    // Get today's date at midnight to avoid time comparison issues
    DateTime today = DateTime(now.year, now.month, now.day);

    // Find the most recent Friday
    DateTime lastFriday = today.subtract(
      Duration(days: (today.weekday - DateTime.friday) % 7),
    );

    // Calculate sunset time for that Friday
    DateTime fridaySunset = _getSunsetTimeForDate(
      lastFriday,
      offset ?? utcOffset,
    );

    // If we're already past this Friday's sunset, look to next Friday
    if (now.isAfter(fridaySunset)) {
      lastFriday = lastFriday.add(const Duration(days: 7));
      fridaySunset = _getSunsetTimeForDate(lastFriday, offset ?? utcOffset);
    }
  ...
  DateTime _getCurrentSabbathEnd(DateTime now, [Duration? offset]) {
    DateTime sabbathStart = _getCurrentSabbathStart(now, offset ?? utcOffset);
    DateTime saturday = sabbathStart.add(const Duration(days: 1));
    return _getSunsetTimeForDate(saturday, offset ?? utcOffset);
  }

  DateTime _getNextSabbathStart(DateTime now, [Duration? offset]) {
    DateTime nextFriday = _getCurrentSabbathStart(now, offset ?? utcOffset);
    // If the current Sabbath start is in the future, the next one is 7 days later
    if (nextFriday.isAfter(now)) {
      return nextFriday;
    }
    return nextFriday.add(const Duration(days: 7));
  }
  ...
  void _updateCountdowns() {
    ...
    final isSabbath =
        now.isAfter(currentSabbathStart) && now.isBefore(currentSabbathEnd);
    final sabbathEnded = now.isAfter(currentSabbathEnd);
    ...
    setState(() {
      ...
      if (isSabbath) {
        // Sabbath has begun - show countdown to end
        sabbathBeginDuration = Duration.zero;
        sabbathEndDuration = currentSabbathEnd.difference(now);
      } else if (sabbathEnded) {
        // Sabbath has ended - show countdown to next Sabbath
        final nextSabbathStart = _getNextSabbathStart(now, currentOffset);
        final nextSabbathEnd = _getNextSabbathEnd(now, currentOffset);
        sabbathBeginDuration = nextSabbathStart.difference(now);
        sabbathEndDuration = nextSabbathEnd.difference(now);
      } else {
        // Sabbath hasn't started yet - show countdown to start and end
        sabbathBeginDuration = currentSabbathStart.difference(now);
        sabbathEndDuration = currentSabbathEnd.difference(now);
      }
    });
  }
  ...
  Widget _sabbathCountdown(Duration duration) {
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              _sabbathCountdown(sabbathEndDuration),
                            ],
                          ),
                        ),
                      ],
