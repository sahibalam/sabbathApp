...
Future<void> _playAlarm() async {
  ...
  try {
    // 1️⃣ Stop any existing playback (prevents overlaps)
    await _audioPlayer.stop();

    // 2️⃣ iOS Audio Session Configuration (unchanged)
    await _audioPlayer.setAudioContext(
      AudioContext(
        ...
        // ✅ Add minimal Android config (silent fail if unsupported)
    ...
    // 3️⃣ Force volume to max (critical for alarms)
    await _audioPlayer.setVolume(1.0);

    // 4️⃣ Play in LOOP mode (unchanged)
    await _audioPlayer.setReleaseMode(ReleaseMode.loop);

    // 5️⃣ Start playback with explicit mode (iOS/Android compatible)
    await _audioPlayer.play(
      AssetSource('sounds/notification.mp3'),
      mode: PlayerMode.mediaPlayer, // Works on both platforms
    );
    ...
    if (_isPlayingAlarm && mounted) await _playAlarm();
  } finally {
  ...
  void initState() {
    ...
    _audioService = AudioReminderService();
      ...
      _audioService.onAudioStateChanged = (isPlaying, remainingSeconds) {
        if (mounted) {
          setState(() {
            ...
            _audioRemainingSeconds = remainingSeconds;
          });
        }
      };
      
      // Recover any ongoing audio playback
      await _audioService.recoverPlaybackState();
    });
  }
   ...
   if (testSebastianMode) {
      ...
      await notificationPlugin.initialize(
        ...
        onDidReceiveNotificationResponse: (details) async {
          ...
          if (details.payload == 'alarm' || details.payload == 'sabbath_reminder_audio') {
            // Instead of playing a short alarm, trigger the long audio reminder
            String title = 'Sabbath Reminder';
            String message = 'Audio reminder is now playing';
            
            // Determine the type of reminder from notification content
            if (details.notificationResponse?.contains('Starts Soon') == true) {
              title = 'Sabbath Starting';
              message = 'Sabbath is beginning soon';
            } else if (details.notificationResponse?.contains('Ends Soon') == true) {
              title = 'Sabbath Ending';
              message = 'Sabbath is ending soon';
            } else if (details.notificationResponse?.contains('Has Started') == true) {
              title = 'Sabbath Started';
              message = 'Shabbat Shalom!';
            } else if (details.notificationResponse?.contains('Has Ended') == true) {
              title = 'Sabbath Ended';
          ...
          } else if (details.payload == 'stop_audio') {
      ...
      await _audioService.initialize(notificationPlugin);
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            // Add audio control widget if audio is playing
            if (_isAudioPlaying) _buildAudioControl(),
...
Future<void> _scheduleWithVerification({
  ...
  try {
    ...
    await notificationPlugin.zonedSchedule(
      ...
      payload: 'sabbath_reminder_audio', // Changed payload to trigger audio
...
Widget _buildAudioControl() {
  return Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border.all(color: const Color(0xFFF4732F), width: 2),
    ),
    child: Row(
      children: [
        const Icon(
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                '${_audioRemainingSeconds}s remaining',
                style: const TextStyle(
